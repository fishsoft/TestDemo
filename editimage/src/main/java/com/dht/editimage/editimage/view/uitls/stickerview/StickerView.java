package com.dht.editimage.editimage.view.uitls.stickerview;
...
public class StickerView extends View {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (!ret && currentItem != null && currentStatus == STATUS_IDLE) {// 没有贴图被选择
                    currentItem.isDrawHelpTool = false;
                    currentItem = null;
                    pictureselectionfalse();//图片没有被选中事件
                    invalidate();
                }
    ...
    //图片没有被选中
    public void pictureselectionfalse() {
        if (stirckerFragment != null) {
            stirckerFragment.toFragfalse();
        }
    }

    //图片被选中
    public void pictureselectionfalsetrue() {
        if (stirckerFragment != null) {
            stirckerFragment.toFragtrue();
        }
    }

    //图片透明度
    public void TranBitmap(int number) {
        currentItem.initbit(number);
        this.invalidate();
    }
    ...
    //隐藏拖动栏
    public void toFragmentMethod() {
        if (stirckerFragment != null) {
            stirckerFragment.hidentSeekBar();
        }
    }


    public void setStirckerFragment(StirckerFragment stirckerFragment) {
